...
public class SpanStringUtils {
	...
	public static SpannableString getEmotionContent(final Context context, final TextView tv, String source) {
		...
		while (matcherEmotion.find()) {
			...
			ArrayMap<Integer,ArrayMap<String, Integer>> ALL_MAP=EmotionUtils.ALL_MAP;
			for (Integer mapkey : ALL_MAP.keySet()) {
				System.out.println("key= "+ mapkey + " and value= " + ALL_MAP.get(mapkey));


				boolean isContain = ALL_MAP.get(mapkey).containsKey(key);
				if (isContain){

					// 利用表情名字获取到对应的图片
					Integer imgRes = EmotionUtils.getImgByName(mapkey,key);
					if (imgRes != null) {
						// 压缩表情图片
						int size = (int) tv.getTextSize()*13/10;
						Bitmap bitmap = BitmapFactory.decodeResource(res, imgRes);
